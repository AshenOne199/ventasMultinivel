...
public class ProductoService {
	...
	public List<ProductoPorRegionDto> getProductosPorRegionYCategoria(Short idRegion, Short idCategoria) {
		...
		List<Inventario> inventarios = inventarioRepository.findByidRegion_idAndIdProducto_idCategoria_id(idRegion, idCategoria);
		for (Inventario inventario : inventarios) {
			ProductoPorRegionDto productoPorRegionDto = new ProductoPorRegionDto(inventario.getId().getkIdproducto(), inventario.getIdProducto().getnNombre(), inventario.getStock(), inventario.getPrecioProducto(), inventario.getIdProducto().getnUrlimagen());
	...
	public List<ProductoPorRegionDto> getProductosPorRegionYSubcategoria(Short idRegion, Short idSubCategoria) {
		List<ProductoPorRegionDto> productoPorRegionDtos = new ArrayList<>();
		List<Inventario> inventarios = inventarioRepository.findByidRegion_idAndIdProducto_idCategoria_idSubcategoria(idRegion, idSubCategoria);
		for (Inventario inventario : inventarios) {
			ProductoPorRegionDto productoPorRegionDto = new ProductoPorRegionDto(inventario.getId().getkIdproducto(), inventario.getIdProducto().getnNombre(), inventario.getStock(), inventario.getPrecioProducto(), inventario.getIdProducto().getnUrlimagen());
			productoPorRegionDtos.add(productoPorRegionDto);
		}
